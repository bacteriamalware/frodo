...
{ // Generate-and-multiply: generate matrix A (N x N) row-wise, multiply by s on the right.
  ...
  // Output: out = A*s + e (N x N_BAR)
	#if defined(USE_AES128_FOR_A)     // Matrix A generation using    AES128, done per 16*N-bit row with multiplication of Matrix S
	...
	#elif defined USE_CSHAKE_FOR_A    // Matrix A generation using cSHAKE128, done per 16*N-bit row with multiplication of Matrix S
    uint16_t a_row[PARAMS_N] = {0};
    int r_A;
...
{ // Generate-and-multiply: generate matrix A (N x N) column-wise, multiply by s' on the left.
  ...
  // Output: out = s'*A + e' (N_BAR x N)
	...
	for(c_A=0; c_A<PARAMS_N; c_A+=PARAMS_STRIPE_STEP) //iterating over columns of Matrix A, operating on 8 (PARAMS_STRIPE_STEP) columns at a time
	{
  ...
  // Output: out = b*s (N_BAR x N_BAR)
    ...
    for (i = 0; i < PARAMS_NBAR; i++)
    {
        for (j = 0; j < PARAMS_NBAR; j++)
        {
            ...
            for (k = 0; k < PARAMS_N; k++)
            {
                out[i*PARAMS_NBAR + j] += b[i*PARAMS_N + k] * s[k*PARAMS_NBAR + j];//old: s[j*PARAMS_N + k];
            }
