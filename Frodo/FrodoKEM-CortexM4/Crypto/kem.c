...
{ // Frodo-KEM's key decapsulation
    ...
    frodo_sample_n(temp, PARAMS_N*PARAMS_NBAR, G, CRYPTO_BYTES, 4);
	#endif

    // Generate Epp, and compute W = Sp*B + Epp
    frodo_sample_n(Epp, PARAMS_NBAR*PARAMS_NBAR, G, CRYPTO_BYTES, 6);
    frodo_unpack(BBp, PARAMS_N*PARAMS_NBAR, sk+CRYPTO_BYTES+BYTES_SEED_A, CRYPTO_PUBLICKEYBYTES - BYTES_SEED_A, PARAMS_LOGQ);

	#if defined USE_FRODO_640
    frodo_mul_add_sb_plus_e(W, BBp, Sp, Epp);
	#elif defined USE_FRODO_976
    frodo_mul_add_sb_plus_e(W, BBp, temp, Epp);
	#endif
	...
	#elif defined USE_FRODO_976
    frodo_mul_add_sa_plus_e(BBp, temp, seed_A);
	#endif
